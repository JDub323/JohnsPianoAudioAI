...
def get_raw_wavs(configs, segment_length: int) -> list[np.ndarray]:
    ...
    # get all wavs
    ret = []
    assert row_count != -1, "Should not use every row in the dataset"
...
def download_cut_labels(configs, segment_length: int) -> list[NoteLabels]:
    # download dataframe which has paths to raw audio files
    df = get_raw_data_df(configs)
...
def plot_spec_and_notes(spec, onset_mat, down_mat, vel_mat, sr: int, hop_length=512, filename=""):
    """
    Plot log-mel spectrogram + onset/down/velocity matrices stacked & aligned.
